...
import twitterapp.textproc as txtpro
...
class TwitterApiUtil():
    ...
    def get_tweets_from_search(self, query, n_items_search=1000, n_tweets_per_user=100, lang="en"):
        ...
        final_dict = {"query": query.encode("utf-8", errors="ignore"), "users": tw_dict}
        return (final_dict)

    def dict_text_processing(self, d, query_filter, lang="en", stopwords=True, POStagfilter=True, stemming=False,
                             lemmatization=True):
        time.sleep(1)
        print("Processing tweets' text...")
        pbar = ProgressBar(widgets=[Percentage(), ' ', Bar(), ' ', ETA()])
        for dict in pbar(d["users"]):
            dict["text"] = txtpro.tweet_to_words("\n".join(dict["text"]), query_filter,
                                                 lang, stopwords, POStagfilter,
                                                 stemming, lemmatization)
        return (d)

    def get_tweets_from_search_cleaned(self, query, n_items_search=1000, n_tweets_per_user=100, lang="en",
                                       stopwords=True, POStagfilter=True, stemming=False, lemmatization=True):
        return (self.dict_text_processing(d=self.get_tweets_from_search(query, n_items_search, n_tweets_per_user, lang),
                                          query_filter=query.encode("utf-8", errors="ignore"), lang=lang, stopwords=stopwords,
